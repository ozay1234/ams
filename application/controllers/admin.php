...
class Admin extends CI_Controller {
	...
	public function surat_masuk() {
		...
		if ($mau_ke == "del") {
			...
			$a['data']		= $this->db->query("SELECT * FROM t_surat_masuk WHERE perihal LIKE '%$cari%' OR asal LIKE '%$cari%' OR no_surat LIKE '%$cari%' ORDER BY id DESC")->result();
			...
			if ($this->upload->do_upload('file_surat')) {
				...
				$this->db->query("INSERT INTO t_surat_masuk VALUES (NULL, '$kode', '$no_agenda', '$perihal', '$asal', '$tujuan', '$no_surat', '$tgl_surat', NOW(), '$ket', '".$up_data['file_name']."', '".$this->session->userdata('admin_id')."')");
			} else {
				$this->db->query("INSERT INTO t_surat_masuk VALUES (NULL, '$kode', '$no_agenda', '$perihal', '$asal', '$tujuan', '$no_surat', '$tgl_surat', NOW(), '$ket', '', '".$this->session->userdata('admin_id')."')");
			}	
			
			$this->session->set_flashdata("k", "<div class=\"alert alert-success\" id=\"alert\">Data sudah di tambahkan ".$this->upload->display_errors()."</div>");
			redirect('index.php/admin/surat_masuk');
		} else if ($mau_ke == "act_edt") {
			if ($this->upload->do_upload('file_surat')) {
				$up_data	 	= $this->upload->data();
							
				$this->db->query("UPDATE t_surat_masuk SET kode = '$kode', no_agenda = '$no_agenda', perihal = '$perihal', asal = '$asal', tujuan = '$tujuan', no_surat = '$no_surat', tgl_surat = '$tgl_surat', keterangan = '$ket', file = '".$up_data['file_name']."' WHERE id = '$idp'");
			} else {
				$this->db->query("UPDATE t_surat_masuk SET kode = '$kode', no_agenda = '$no_agenda', perihal = '$perihal', asal = '$asal', tujuan='$tujuan', no_surat = '$no_surat', tgl_surat = '$tgl_surat', keterangan = '$ket' WHERE id = '$idp'");
			}	
			
			$this->session->set_flashdata("k", "<div class=\"alert alert-success\" id=\"alert\">Data sudah di perbaharui ".$this->upload->display_errors()."</div>");			
			redirect('index.php/admin/surat_masuk');
		} else {
	...
	public function surat_keluar() {
		...
		if ($mau_ke == "del") {
			...
			$a['data']		= $this->db->query("SELECT * FROM t_surat_keluar WHERE perihal LIKE '%$cari%' OR tujuan LIKE '%$cari%' OR no_surat LIKE '%$cari%' ORDER BY id DESC")->result();
			...
			if ($this->upload->do_upload('file_surat')) {
				...
				$this->db->query("INSERT INTO t_surat_keluar VALUES (NULL, '$kode', '$no_agenda', '$perihal', '$tujuan', '$no_surat', '$tgl_surat', NOW(), '$ket', '".$up_data['file_name']."', '".$this->session->userdata('admin_id')."')");
			} else {
				$this->db->query("INSERT INTO t_surat_keluar VALUES (NULL, '$kode', '$no_agenda', '$perihal', '$tujuan', '$no_surat', '$tgl_surat', NOW(), '$ket', '', '".$this->session->userdata('admin_id')."')");
			}		
			
			$this->session->set_flashdata("k", "<div class=\"alert alert-success\" id=\"alert\">Data sudah di tambah </div>");
			redirect('index.php/admin/surat_keluar');
		} else if ($mau_ke == "act_edt") {
			if ($this->upload->do_upload('file_surat')) {
				$up_data	 	= $this->upload->data();
				
				$this->db->query("UPDATE t_surat_keluar SET no_agenda = '$no_agenda', kode = '$kode', perihal = '$perihal', tujuan = '$tujuan', no_surat = '$no_surat', tgl_surat = '$tgl_surat', keterangan = '$ket', file = '".$up_data['file_name']."' WHERE id = '$idp'");
			} else {
				$this->db->query("UPDATE t_surat_keluar SET no_agenda = '$no_agenda', kode = '$kode', perihal = '$perihal', tujuan = '$tujuan', no_surat = '$no_surat', tgl_surat = '$tgl_surat', keterangan = '$ket' WHERE id = '$idp'");
			}	
	...
	public function surat_disposisi() {
		...
		$no_disp				= addslashes($this->input->post('no_disp'));
		...
		if ($mau_ke == "del") {
			...
			$this->session->set_flashdata("k", "<div class=\"alert alert-success\" id=\"alert\">Data sudah di hapus </div>");
			...
			$this->db->query("INSERT INTO t_disposisi VALUES (NULL, '$id_surat', '$no_disp', '$kpd_yth','$dari', '$isi_disposisi', '$sifat', '$catatan')");
			
			$this->session->set_flashdata("k", "<div class=\"alert alert-success\" id=\"alert\">Data sudah di tambahkan</div>");
			redirect('index.php/admin/surat_disposisi/'.$id_surat);
		} else if ($mau_ke == "act_edt") {
			$this->db->query("UPDATE t_disposisi SET no_disp='$no_disp', kpd_yth = '$kpd_yth', dari='$dari' isi_disposisi = '$isi_disposisi', sifat = '$sifat', catatan = '$catatan' WHERE id = '$idp'");
			$this->session->set_flashdata("k", "<div class=\"alert alert-success\" id=\"alert\">Data sudah di perbaharui</div>");			
			redirect('index.php/admin/surat_disposisi/'.$id_surat);
		} else {
	...
	public function pengguna() {
		...
		$kepsek					= addslashes($this->input->post('kepala'));
		$nip_kepsek				= addslashes($this->input->post('nip_kepala'));
	...
	public function manage_admin() {
		...
		if ($mau_ke == "del") {
			...
			if (strlen($username) < 3) {
				$this->session->set_flashdata("k", "<div class=\"alert alert-danger\" id=\"alert\">Username minimal 3 huruf</div>");
			} else if (strlen($pass_raw1) < 3) {
				$this->session->set_flashdata("k", "<div class=\"alert alert-danger\" id=\"alert\">Password minimal 3 huruf</div>");
				...
				$this->db->query("INSERT INTO t_admin VALUES (NULL, '$username', '$password', '$nama', '$nip', '$jabatan', '$level')");
				$this->session->set_flashdata("k", "<div class=\"alert alert-success\" id=\"alert\">Data Sudah Di Tambah</div>");
			}
			...
			if (strlen($username) < 3) {
				...
				$this->db->query("UPDATE t_admin SET username = '$username', nama = '$nama', nip = '$nip', jabatan = '$jabatan', level = '$level' WHERE id = '$idp'");
				$this->session->set_flashdata("k", "<div class=\"alert alert-success\" id=\"alert\">Data Sudah Di Perbarui</div>");
			}
	...
	public function disposisi_cetak() {
		...
		$a['datpil4']	= $this->db->query("SELECT dari FROM t_disposisi WHERE id_surat = '$idu'")->result();
		$a['datpil3']	= $this->db->query("SELECT no_disp, isi_disposisi, sifat FROM t_disposisi WHERE id_surat = '$idu'")->result();	
